...
#
# newproject.sh <project name> <path> <root package name> [compile sdk version] [library project <Y/N>] [main activity name]

if [ "$#" -lt "3" ]
	then
	echo "newproject.sh <project name> <path> <root package name> [compile sdk version] [library project <Y/N>] [main activity name]"
	exit
fi
...
CWD=`pwd`
bash ${CWD}/addgradle.sh "${TARGET_PROJECT_PATH}" "${PROJECT_COMPILE_SDK_VERSION}" "${PROJECT_IS_LIBRARY}"

# Create a default manifest file
echo -e "\n"
echo " Android configuration - Target SDK Version:  ${PROJECT_COMPILE_SDK_VERSION}"
echo "                         Root Package Name :  ${PROJECT_PACKAGE_NAME}"
echo -e "                         Main Activity Name:  ${PROJECT_MAIN_ACTIVITY_NAME}\n"
...
echo -n "   > Generating default strings.xml"
